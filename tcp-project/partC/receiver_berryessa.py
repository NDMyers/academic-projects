...
# Get arguments
if (len(sys.argv) != 13 or sys.argv[1] != "--ip" or sys.argv[3] != "--port" or sys.argv[5] != "--packet_loss_percentage" or sys.argv[7] != "--round_trip_jitter" or sys.argv[9] != "--bdp" or sys.argv[11] != "--output"):
    print("usage: python3 server_putah.py --ip XXXX.XXXX.XXXX.XXXX --port YYYY --packet_loss_percentage X --round_trip_jitter Y --bdp Z --output output.txt")
    sys.exit(0)
HOST = str(sys.argv[2])
PORT = int(sys.argv[4])
...
def parseSyn(packet,addr,senderAddr):
    ...
    if syn==1:
        ...
        packet = sport + newdport + seqnum + ackNum + newSlice + packet[15:17]
...
def adjust_seqackNum(packet, bits_sent_so_far):
    ...
    adjusted_packet = packet[2:4] + packet[0:2] + new_seqnum + new_acknum + packet[12:17]
...
def connectionSocket(sock2):
    ...
    while True:
        ...
        try:
            ...
            numBits = len(data)
            cwndSize = parsePacket(data,'cwnd')
            bits_sent_at_one_time = cwndSize*numBits
            ...
            if bits_sent_at_one_time > BDP:
                if random_chance < (3*PACKET_LOSS_PERCENTAGE):
                    continue
                elif random_chance > JITTER:
                    time.sleep(3*(random_chance/100))
            # NORMAL STATE
            elif bits_sent_at_one_time <= BDP:
